	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed:
				...
				has_last_pos = true
				strokes.append({"pos": last_pos, "color": color})
				queue_redraw()
			else:
				has_last_pos = false
		...
		if has_last_pos:
			...
			for i in range(steps):
				...
				strokes.append({"pos": interp_pos, "color": color})
		strokes.append({"pos": event.position, "color": color})
		last_pos = event.position
		has_last_pos = true
		queue_redraw()
...
func _draw() -> void:
	for stroke in strokes:
		draw_circle(stroke.pos, 5, stroke.color)
